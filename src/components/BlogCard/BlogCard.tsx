...
const BlogCard = (props: Props) => {
  ...
  return (
    <button onClick={() => {
        navigate(`/Blog/${props.data.id}-${props.data.title.replace(/ /g, '-')}`, {state: { data: props.data }})
    }} className={`${props.alignment === 'horizontal' ? 'h-36 space-x-6': 'flex-col space-y-6'} group cursor-pointer relative flex ${ props.size === 'normal' && props.alignment === 'vertical' ? 'w-min': 'w-full' } items-center justify-start`}>
        <div className={`${props.alignment === 'horizontal' ? 'min-w-60 min-h-full': props.size === 'normal' ? 'min-w-60 min-h-36': 'min-w-full min-h-60'} overflow-hidden relative`}>
        ...
        <div className=" text-left space-y-3">
            ...
            <h1 className="text-xl text-ellipsis">{props.data.title}</h1>
            <p className={`${ props.alignment === 'vertical' ? 'line-clamp-6': 'line-clamp-3' } text-sm text-ellipsis`}>{props.data.description}</p>
        </div>
